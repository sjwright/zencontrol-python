...
async def test_configure_controller_events_attaches_hot_plugged_controller() -> None:
    ...
    assert zen.wiring is not None
    assert zen.wiring.get(ctrl) is not None
...
async def test_attach_once_survives_forced_endpoint_death() -> None:
    ...
    await zen.start()
    assert zen.wiring is not None
    zen.wiring.on_resync = on_resync

    binding = zen.wiring.get("ctrl-a")
    ...
    assert zen.wiring.get("ctrl-a") is first_binding
...
async def test_host_only_binding_learns_mac_and_persists() -> None:
    ...
    binding = zen.wiring.get("pending") if zen.wiring else None
    assert binding is not None
...
async def test_promotion_conflict_detaches_zombie_binding() -> None:
    ...
    assert zen.wiring.get("pending") is None
